...
EXTEND SPLGram
  ...
  test: [[e1 = expr; `SEMICOLON -> e1]];

  row: [[`ROWWORD; es = LIST1 test; `ENDWORD -> es]];

  tag: [[`IDENTIFIER s; `COLON; t=typ -> (s, t)]];

  proj_tag: [[`IDENTIFIER s -> s]];

  reltype: [[`OBRACE; `RELTYPEWORD; t = LIST1 tag; `ENDWORD; `CBRACE -> RelType (t)]];
     ...
     [ LEFTA
	  ...
	  [e1 = SELF;  "&";e2 = SELF -> BinaryPrimApp ("&",e1,e2)]
       |   "Equality" LEFTA
        ...
        | "Proj Join" LEFTA
            [e1 = SELF; `JOINOP; e2 = SELF -> BinaryPrimApp ("@", e1, e2)
            | e1 = SELF; `PROJECTOP; e2 = LIST1 proj_tag -> Proj (e1, e2)]
         | "Bracket" LEFTA
           ["("; e=SELF; ")" -> e]
         | "Unary" NONA
       ...
       (*    [e1 = SELF; `DIV ;  e2 = SELF -> BinaryPrimApp ("/",e1,e2) ] *)
        ...
        | `RELWORD; t = reltype; r = LIST1 row; `ENDWORD  -> Rel(t, r)
        |  `RECFUN; `IDENTIFIER s; `OBRACE;t = typ;`CBRACE; al = args; `RARROW; e = SELF; `ENDWORD -> RecFunc(t,s,al,e)
